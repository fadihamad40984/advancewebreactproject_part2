...
import L from "leaflet";
import Chart from "chart.js/auto";
import villagesData from "/data/villages.json"; // Assuming you still want initial data as fallback
...
const Overview = () => {
  ...
  const [stats, setStats] = useState({
    ...
    averageLandArea: '0.00 sq km'
  });

  useEffect(() => {
    // Fetch villages data from localStorage or fallback to the initial villagesData
  ...
  useEffect(() => {
    const initializeCharts = () => {
      ...
      setStats({
        totalVillages,
        totalUrbanAreas,
        totalPopulationSize: totalPopulation.toLocaleString(),
        averageLandArea: `${averageLandArea} sq km`
      });
      ...
      villages.forEach((village) => {
        ...
        if (demographic && demographic.ageDistribution) {
          const ageDistribution = demographic.ageDistribution
            ...
            .map((dist) => parseInt(dist.split(":")[1], 10));
          aggregatedAgeDistribution = aggregatedAgeDistribution.map(
            (sum, idx) => sum + (ageDistribution[idx] || 0)
          );
        } else {
          aggregatedAgeDistribution = aggregatedAgeDistribution.map(() => 0);
        }
    ...
    <Dashboard>
      <div className="main-page">
        <div className="overview">
          ...
          <div id="map" ref={mapRef}></div>
          <div className="stats">
            <div id="totalVillages">{`Total Number of Villages: ${stats.totalVillages}`}</div>
            <div id="totalUrbanAreas">{`Total Number of Urban Areas: ${stats.totalUrbanAreas}`}</div>
            <div id="totalPopulationSize">{`Total Population Size: ${stats.totalPopulationSize}`}</div>
            <div id="averageLandArea">{`Average Land Area: ${stats.averageLandArea}`}</div>
          </div>
        </div>
        <div className="charts">
          <div className="chart-container">
